...
namespace ET.Client
{
    public class CM_Bias_BBScriptHandler : BBScriptHandler
    {
        ...
        //CM_Bias: DefaultCamera, 30, 30;
        public override async ETTask<Status> Handle(BBParser parser, BBScriptData data, ETCancellationToken token)
        {
            Match match = Regex.Match(data.opLine, @"CM_Bias: (?<Camera>\w+), (?<CenterX>.*?), (?<CenterY>.*?);");
