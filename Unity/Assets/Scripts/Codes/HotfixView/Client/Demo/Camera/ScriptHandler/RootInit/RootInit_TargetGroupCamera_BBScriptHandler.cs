...
namespace ET.Client
{
    ...
    public class RootInit_TargetGroupCamera_BBScriptHandler : BBScriptHandler
    {
        ...
        {
            Match match = Regex.Match(data.opLine, @"TargetGroupCamera: (?<Name>\w+);");
            if (!match.Success)
            {
            ...
            //5. 初始化TargetGroup组件
            CinemachineTargetGroup targetGroup = go.AddComponent<CinemachineTargetGroup>();
            targetGroup.AddMember(vc.Follow, 1, 3);
