...
namespace ET.Client
{
    public class Function_SpawnADust_BBScriptHandler : BBScriptHandler
    {
        ...
        {
            Match match = Regex.Match(data.opLine, "SpawnADust: (?<posX>.*?), (?<posY>.*?), (?<scaleX>.*?), (?<scaleY>.*?);");
            if (!match.Success)
            {
            ...
            go.transform.rotation = parent.transform.rotation * Quaternion.Euler(0, 0, 90); // 将Parent局部空间中的旋转(0, 0, 90)转换至世界空间的旋转
