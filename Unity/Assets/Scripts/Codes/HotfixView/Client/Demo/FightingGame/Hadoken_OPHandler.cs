namespace ET.Client
{
    ...
    public class Hadoken_OPHandler
    {
        ...
        public async ETTask<Status> CheckHandler(Unit unit, ETCancellationToken token)
        {
            ...
            //第二段
            InputBuffer status_2 = await inputWait.Wait(op: TODOperaType.DOWNRIGHT, cancellationToken: token, waitType: 0, waitFrame: 5);
            if (status_2.Error != WaitTypeError.Success) return Status.Failed;
            
            //第三段
            InputBuffer status_3 = await inputWait.Wait(op: TODOperaType.RIGHT | TODOperaType.UPRIGHT, cancellationToken: token, waitType: 0, waitFrame: 5);
            if (status_3.Error != WaitTypeError.Success) return Status.Failed;
           
            //等待按下 LP + MP / LP + HP / MP + HP
            InputBuffer status_4 = await inputWait.Wait(op: TODOperaType.LIGHTPUNCH | TODOperaType.MIDDLEPUNCH | TODOperaType.HEAVYPUNCH, cancellationToken: token, waitType: 1, waitFrame: 5);
