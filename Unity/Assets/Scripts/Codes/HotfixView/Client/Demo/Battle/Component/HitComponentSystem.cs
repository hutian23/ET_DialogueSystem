using System.Collections.Generic;
using ET.Event;

namespace ET.Client
{
    [FriendOf(typeof(HitComponent))]
    public static class HitComponentSystem
    {
        public class HitComponentAwakeSystem : AwakeSystem<HitComponent>
        {
            protected override void Awake(HitComponent self)
            {
                self.Init();
            }
        }

        [FriendOf(typeof(b2Body))]
        [FriendOf(typeof(BBParser))]
        public class HitComponentPostStepSystem : PostStepSystem<HitComponent>
        {
            protected override void PosStepUpdate(HitComponent self)
            {
                Unit unit = self.GetParent<BBParser>().GetParent<Unit>();
                b2Body body = b2WorldManager.Instance.GetBody(unit.InstanceId);

                Queue<CollisionBuffer> bufferQueue = body.triggerStayBuffers;
                int count = bufferQueue.Count;
                while (count-- > 0)
                {
                    CollisionBuffer buffer = bufferQueue.Dequeue();
                    bufferQueue.Enqueue(buffer);

                    //1. 获取碰撞双方的判定框信息
                    b2Box boxA = Root.Instance.Get(buffer.instanceIdA) as b2Box;
                    b2Box boxB = Root.Instance.Get(buffer.instanceIdB) as b2Box;
                    if (boxA.GetBoxType() is not HitboxType.Hit || boxB.GetBoxType() is not HitboxType.Hurt) continue;

                    //2. 组件查询
                    b2Body bodyA = boxA.GetParent<b2Body>();
                    b2Body bodyB = boxB.GetParent<b2Body>();
                    Unit unitA = Root.Instance.Get(bodyA.unitId) as Unit;
                    Unit unitB = Root.Instance.Get(bodyB.unitId) as Unit;

                    //3. 已经触发过攻击回调，是否再次调用?
                    if (self.buffSet.Contains(unitB.InstanceId) && self.checkType.Equals("Once")) continue;
                    self.buffSet.Add(unitB.InstanceId);

                    //4. 触发攻击回调
                    BBParser parser = unitA.GetComponent<BBParser>();
                    self.buffer = buffer;
                    parser.RegistSubCoroutine(self.startIndex, self.endIndex, parser.CancellationToken).Coroutine();
                    self.buffer = default;
                }
            }
        }

        public class HitComponentDestroySystem : DestroySystem<HitComponent>
        {
            protected override void Destroy(HitComponent self)
            {
                self.Init();
            }
        }

        private static void Init(this HitComponent self)
        {
            self.startIndex = 0;
            self.endIndex = 0;
            self.checkType = string.Empty;
            self.buffSet.Clear();
            self.buffer = default;
        }
        
        public static CollisionBuffer GetBuffer(this HitComponent self)
        {
            return self.buffer;
        }
    }
}
