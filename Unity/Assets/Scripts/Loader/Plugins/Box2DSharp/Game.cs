    ...
    public class Game: MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            var allTypes = this.GetType()
                    .Assembly.GetTypes();
            var testTypeArray = allTypes.Where(e => testBaseType.IsAssignableFrom(e)
                            && !e.IsAbstract
                            && e.GetCustomAttribute<TestCaseAttribute>() != null)
                    .ToArray();
            var testTypes = new HashSet<Type>(testTypeArray);
            var inheritedTest = allTypes.Where(e => testBaseType.IsAssignableFrom(e)
        ...
        public void CheckKeyDown()
        {
            ...
            else
            {
                foreach (var keyCode in UnityInput.KeyCodeMap)
                {
                    ...
                    {
                        Test?.OnKeyDown(new KeyInputEventArgs(keyCode.Key,
        ...
        private void CheckKeyUp()
        {
            ...
            foreach (var keyCode in UnityInput.KeyCodeMap)
            {
                ...
                {
                    Test?.OnKeyUp(new KeyInputEventArgs(keyCode.Key,
                        UnityInput.GetKeyModifiers(key),
                        false));
                }
            }
